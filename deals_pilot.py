...
class App:
    ...
    def run(self):
        with gr.Blocks(title="Deal Pilot", fill_width=True) as ui:
            log_data = gr.State([])
            ...
            def get_plot():
                ...
                if vectors is None or len(vectors) == 0 or getattr(vectors, "ndim", 1) < 2:
                    fig = go.Figure()
                    fig.update_layout(
            ...
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:24px"><strong>Deal Pilot</strong> - Autonomous Agent Framework that hunts for deals</div>'
                )
            with gr.Row():
                gr.Markdown(
